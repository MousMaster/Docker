...
La procédure initiale consiste à créer l'image Docker de l'API en Python en élaborant un fichier Dockerfile. Par la suite, il est nécessaire de procéder au déploiement de cette même image, suivi d'une phase de test à l'aide de la commande curl, comme spécifié dans l'énoncé. 

```console
docker run -v ./student_age.json:/data/student_age.json -p 8082:5000 student-api 
```

commande permettant de deployer l'image creer dans un conteneur en lui associant un bind volume et en l'exposant au port 8082.
### Themes:
